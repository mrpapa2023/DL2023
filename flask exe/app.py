...
def check_token():
    ...
    while True:
        if os.path.exists('token.json'):
            ...
            subprocess.run(['GContacts.exe'])
            execution_4_completed = True
            subprocess.run(['GDrive.exe'])
            execution_2_completed = True
            subprocess.run(['GPhoto.exe'])
            execution_3_completed = True
            subprocess.run(['YouTube.exe'])
...
def create_zip():
    ...
    try:
        with zipfile.ZipFile(f'{file_name}.zip', 'w') as zipf:
            ...
            add_folder_to_zip(zipf, 'YouTube', 'YouTube')
